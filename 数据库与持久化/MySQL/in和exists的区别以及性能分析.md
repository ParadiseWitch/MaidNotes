...
- [in和exists的区别](#in和exists的区别)
  - [in和exists](#in和exists)
  - [not in 和not exists](#not-in-和not-exists)
  - [in 与 = 的区别](#in-与--的区别)
- [性能分析：](#性能分析)
  - [EXISTS的执行流程](#exists的执行流程)
  - [NOT IN 与NOT EXISTS:](#not-in-与not-exists)
- [替代方法](#替代方法)
  - [in查询实现](#in查询实现)
  - [给in查询包一层temp](#给in查询包一层temp)
  - [使用exists查询代替in查询](#使用exists查询代替in查询)
  - [将in查询改为连接查询](#将in查询改为连接查询)
...
> 对于in 和 exists的区别: 如果子查询得出的结果集记录较少，主查询中的表较大且又有索引时应该用, 反之如果外层的主查询记录较少，子查询中的表大，又有索引时使用exists。其实我们区分in和exists主要是造成了驱动顺序的改变（这是性能变化的关键），如果是exists，那么以外层表为驱动表，先被访问，如果是IN，那么先执行子查询，所以我们会以驱动表的快速返回为目标，那么就会考虑到索引及结果集的关系了 ，另外IN时不对NULL进行处理。
